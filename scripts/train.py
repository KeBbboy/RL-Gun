...
def train(args):
    ...
    while len(episode_rewards) <= train_cfg['num_episodes']:
        ...
        cur_eps = get_epsilon(train_step, warmup_steps=ws)
        ...
        # 可视化
        if features.get('visualize', False) and len(episode_rewards) % features.get('visualize_interval', 1) == 0:
            env.visualizer.visualize_step(
        ...
        if done:
            ...
            # 保存模型
            if len(episode_rewards) % config['logging'].get('save_rate', 2000) == 0 and len(episode_rewards) > 1:
                save_dir = config['logging']['save_dir']
                os.makedirs(save_dir, exist_ok=True)
                
                for i, agent in enumerate(trainers):
                    if algorithm_name == "ma2c":
                        agent.policy.save_weights(f"{save_dir}/agent_{i}_policy_ep{len(episode_rewards)}.h5")
                    elif algorithm_name == "mappo":
                        agent.policy.save_weights(f"{save_dir}/agent_{i}_mappo_policy_ep{len(episode_rewards)}.weights.h5")
                    else:
                        agent.actor.model.save_weights(f"{save_dir}/agent_{i}_actor_ep{len(episode_rewards)}.h5")
                        agent.critic.model.save_weights(f"{save_dir}/agent_{i}_critic_ep{len(episode_rewards)}.h5")
                
                logger.log_text(f"Model saved at episode {len(episode_rewards)}")
