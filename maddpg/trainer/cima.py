...
from maddpg.trainer.per_buffer import PrioritizedReplayBuffer
from maddpg.trainer.model import ActorModel, CriticModel
from maddpg.trainer.utils import update_target_weights
...
class TruckPolicyTrainer:
    ...
    def train(self, obs, all_acts, global_obs, critic_fn):
        """训练方法"""
...
class MADDPGAgentTrainer(AgentTrainer):
    ...
    def update(self, agents, t, **kwargs):
        ...
        # 其余 agent 固定；PolicyTrainer.train 内部会替换本智能体动作
        all_acts_const = [tf.stop_gradient(a) for a in all_agents_act_onehots]
        actor_loss = self.actor.train(obs_tensor, all_acts_const, glob_tensor, self.critic.model)
